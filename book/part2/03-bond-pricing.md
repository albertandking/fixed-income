...
**本章是全书的第一块基石。** 第1章介绍了债券这一"事先约定现金流的合同"，第2章讨论了货币的时间价值与计息惯例。本章要回答最核心的问题：**一只债券究竟值多少钱？** 答案十分朴素——**债券的价格，就是它未来全部现金流按合理利率折算到今天的现值之和**。本章将这一论断写成公式、用 Python 实现，再用真实的中国国债加以验证。掌握这一原理，便掌握了后续各章（收益率、久期、利差、衍生品）的总线索。
...
**一价定律之所以必然成立，可用反证法（套利论证）说明**：假设一只债券的市价**低于**其现金流现值，套利者即可"买入这只被低估的债，同时卖出能复制其现金流的零息债组合"，从而锁定无风险利润；市价高于现值时反向操作亦然。逐利的套利活动会持续推动价格回归现值——唯有当市价恰好等于现值时，套利机会方才消失。**由此可见，正是套利机制约束着价格等于现值**。这套"复制加无套利"的逻辑，是整个固定收益乃至衍生品定价的总纲。
...
之所以要求出这个闭式解、而非逐笔相加，是因为它将"一串现金流"压缩成一个**只含四个量（票息、收益率、期数、面值）的紧凑表达式**，从而清晰地呈现价格对各个量的依赖关系——这正是后续推导久期（对 $y$ 求导）、分析期限效应的起点。从数学上看，票息部分是一个公比为 $(1+y/k)^{-1}$ 的等比数列，几何级数求和的结果即为"$\frac{1-(1+y/k)^{-N}}{y/k}$"这一年金因子；它在金融上称为**年金现值系数**，含义是"未来每期 1 元、连续 $N$ 期，折算到今天的现值"。从**时间视角**看，债券价格是将一条"时间轴上高低错落的现金流"全部贴现、汇聚到今天这一时点上的总和——越远的现金流折现幅度越大，因此**在长债的价格中，本金一笔的权重被时间稀释而显著降低，票息累积的分量则相对更重**。这一直觉在第6章将表述为"久期即现金流的时间重心"。
...
这是固定收益领域最根本的一条直觉——**利率上行，债券价格下跌**。不借助公式亦可理解：一只债的票息在发行时即已**固定**，例如 3%。若市场上新发的同类债当前提供 4% 的收益率，投资者便不会按原价买入这只"仅提供 3%"的旧债——它必须**降价**，直至"折价买入加 3% 票息"折算下来的总回报也能达到 4%，方有人承接。反之，当市场利率下行至 2% 时，这只票息固定为 3% 的债便更具吸引力，价格随之被推高。**换言之，价格的涨跌，本质上是市场以价差将一只"票息固定的旧债"重新校准至"当前的市场收益率"。** 这条曲线并非直线，而是向原点凸出的（凸性，第6章），但其单调下降的方向始终成立。

这条规律贯穿于债券市场的日常运行之中。2022 年，美联储在一年内将基准利率从接近零的水平上调至 4% 以上，存量长债价格随之大幅下跌。美国硅谷银行（SVB）因重仓持有长久期国债与 MBS，在加息过程中累积了大量浮亏，2023 年遭遇挤兑时不得不折价变现资产，最终破产。"利率上行、债券价格下跌"看似是一句寻常的教科书结论，却足以撼动一家银行：**理解价格与收益率的反向关系，其意义不止于应试，更在于把握利率周期中潜在的风险敞口。**
...
!!! warning "误区：票面利率 ≠ 收益率"
    "票息 5% 的债，收益率即为 5%"——这一说法并不成立。**票面利率（coupon rate）**是发行时确定、用以计算票息的固定数字，载于债券条款且不再改变；**到期收益率（yield）**则是市场依据当前价格随时变动的折现率。只有当债券**恰好平价**时两者才相等。溢价买入一只高票息债时，投资者的实际收益率低于票面利率（因为多付的溢价会随拉回面值而损耗）。将票面利率等同于投资回报，是债券投资中最常见的认知误区。
...
区分这两个价格的意义何在？设想若直接以全价报价会出现何种情形：随着付息日临近，应计利息逐日累积，全价被这部分利息持续抬高；至付息日当天票息发放，全价又随之回落——于是行情图上会呈现一条"锯齿"形态：每个付息周期内单调上升，付息日则陡然下降。这一锯齿与债券的真实价值并无关系，纯粹是利息累积所致的会计现象，却会干扰投资者对"该债究竟偏贵还是偏便宜"的判断。**净价的作用，正是将这部分累积的应计利息剥离，使报出的数字只反映市场对债券价值的判断**——如此，付息时点不同的两只债才能在同一标准下加以比较。结算时之所以要补回应计、以全价交收，是因为卖方确实持有了大半个付息周期，理应取得这段时间对应的利息：买方先行垫付，待下一付息日一次性收回整期票息，恰好相抵。**换言之，净价服务于价值的"可比性"，全价服务于交收的"公平性"，二者各司其职。**
...
!!! success "概念题 2"
    **票息 $c$ 与收益率 $y$ 的关系决定状态**：$c>y$ 溢价（$P>$ 面值）、$c=y$ 平价、$c<y$ 折价。**拉回面值**：只要收益率不变，随到期临近，债券价格平滑收敛回面值（到期偿付的就是面值）——溢价债逐步下行至 100、折价债逐步上行至 100。这说明持有期收益不仅来自票息，也来自价格向面值的回归。

!!! success "概念题 3"
    **全价 = 净价 + 应计利息**。净价剔除应计、便于比较；全价是实际交收额。中国债市净价报价（干净可比）、全价结算（买方补付卖方累积未收的利息），兼顾报价清晰与交收公平（详见第2章）。

!!! success "计算题 4"
    3 年期、年付息、票息 4%、面值 100，用闭式解 $P=4\cdot\frac{1-(1+y)^{-3}}{y}+100(1+y)^{-3}$：
    ...
    与 `fi.price_bond` 完全一致，验证了"票息高于收益率则溢价"的规律。

!!! success "计算题 5"
    零息债 $P=F(1+y)^{-T}$，反解 $y$：

    $$y=\left(\frac{100}{88}\right)^{1/5}-1=\mathbf{2.5896\%}$$

    （用 `fi.pricing.ytm(88, [100], [5], freq=1)` 验证得 2.5896%。）

!!! success "编程实验 6–8（要点）"
    - **实验 6**：`price_bond` 复现例3.1（102.88/100/97.22）、例3.2（零息 97.56/92.86）；画 $y\in[0,8\%]$ 的价格曲线，单调下降即"价格—收益率反向变动"。
    - **实验 7**：固定收益率，让剩余期限从 10→0 年，溢价债价格下行、折价债上行，**都收敛到 100**——即图3-2 拉回面值。
    - **实验 8**：fi 与 QuantLib `FixedRateBond` 在 $y=2\%/3\%/4\%$ 的对拍误差应在 $10^{-6}$ 量级（整年、`Unadjusted` 时）；差异来源是**计息惯例（ActualActual）/付息频率/结算日**——引入真实结算日后误差会放大到分位级。
