...
为什么要费力求这个闭式解、而不直接一笔一笔加？因为它把"一串现金流"压缩成了一个**只含四个量（票息、收益率、期数、面值）的紧凑表达式**，让我们一眼看清价格对每个量的依赖关系——这正是后面推导久期（对 $y$ 求导）、分析期限效应的起点。从数学上看，票息部分是一个公比为 $(1+y/k)^{-1}$ 的等比数列，几何级数求和的结果就是那个"$\frac{1-(1+y/k)^{-N}}{y/k}$"的年金因子；它有个朴素的金融名字叫**年金现值系数**，含义是"未来每期 1 元、连续 $N$ 期，今天值多少钱"。换一个**时间视角**：债券价格其实是把一条"时间轴上高低错落的现金流"全部贴现、压到今天这一个点上的总和——越远的现金流被折得越狠，所以**长债的价格里，本金那一笔的权重被时间稀释得很轻，票息累积的分量反而更重**。这个直觉到第6章会变成"久期=现金流的时间重心"。
...
这是固定收益最根本的一条直觉——**利率涨，债券价格跌**。为什么？一个不靠公式的理解是：你手里这只债的票息是发行时**锁死**的，比如 3%。如果市场上新发的同类债现在给 4%，那没人愿意按原价买你这只"只给 3%"的旧债——它必须**降价**，降到"折价买入 + 3% 票息"算下来也能达到 4% 的总回报，才有人接手。反之，市场利率跌到 2% 时，你这只锁死 3% 的债就成了香饽饽，价格被买高。**价格的涨跌，本质是市场在用价差把一只"票息固定的旧债"重新校准到"当前的市场收益率"。** 这条曲线不是直线，而是向原点凸出的（凸性，第6章），但单调下降的方向永远成立。

这条朴素规律是整个债券市场每天的脉搏。2022 年美联储一年内把基准利率从近零拉到 4% 以上，存量长债价格应声暴跌——美国硅谷银行（SVB）正是因为重仓的长久期国债与 MBS 在加息中浮亏累累，2023 年挤兑时被迫亏本变现而倒闭。"利率涨、债券跌"听上去是一句教科书废话，但它能掀翻一家银行：**理解价格与收益率的反向关系，不只是为了考试，而是为了知道利率周期里谁在裸泳。**
...
为什么要多此一举地分出两个价格？想象一下如果直接用全价报价会发生什么：随着付息日临近，应计利息一天天累积，全价会被这部分利息**人为地越垫越高**，到付息日当天利息发放、全价又"咣"地跳水回落——于是行情图上会出现一条荒唐的"锯齿":每个付息周期内单调爬升、付息日断崖下跌。这条锯齿和债券的真实价值毫无关系，纯粹是利息累积的会计假象，却会严重干扰投资者判断"这只债到底是贵了还是便宜了"。**净价的妙处，正是把这段累积的应计利息剥离出去，让报出来的数字只反映市场对债券价值的真实判断**——两只付息时点不同的债，也才能放在同一把尺子上比较。而结算时之所以要补回应计、用全价交收，是因为卖方确实持有了大半个付息周期、理应拿走这段时间挣到的利息——买方先垫付，等到下个付息日一次性把整期票息收回来，刚好抹平。**净价负责"看得清"，全价负责"算得公"，两者各司其职。**
