...
VaR 之所以能横扫全球风控界、成为董事会和监管者都听得懂的语言，是因为它把"这个庞杂组合到底有多危险"这个让人头大的问题，浓缩成了**一句人话、一个数字**："我们有 99% 的把握，明天最多亏 580 万。"CEO 不需要懂久期凸性，看一眼这个数就知道自己睡不睡得着。但恰恰是这份"一个数字打天下"的简洁，藏着 VaR 最致命的陷阱——**它只告诉你门槛在哪里，却对门槛之外的深渊只字不提。** "99% 的日子亏不超过 580 万"这句话,对那剩下 **1% 的日子**完全沉默：那一天可能亏 600 万，也可能亏 6000 万，VaR 一视同仁、闭口不谈。更糟的是，人们用历史数据估出来的"平静期 VaR"会显得很小，于是放心加杠杆，直到那 1% 的尾部真的降临——2008 年金融危机里，无数机构的 VaR 模型在风暴前夜还显示"风险可控",转眼就被远超模型预言的损失击穿。**VaR 给了人一种精确的安全幻觉，而真正杀死你的，往往是它按定义就拒绝去看的那 1%。** 这正是下一节 CVaR 要补上的缺口，也是读这一章时必须时刻警惕的一点：风险度量本身，可能成为最大的风险来源。
...
用一个比喻就能记住二者的分工：**VaR 是悬崖边的一道护栏，它告诉你"栏杆设在离崖边几米处"；CVaR 则告诉你"万一翻过了栏杆，下面的悬崖有多深"。** 一道护栏设得再标准，也不会改变它脚下究竟是三米土坡还是三百米深渊——而对一个真正可能让你万劫不复的组合，深渊的深度远比护栏的位置重要。两个组合可以有一模一样的 VaR（护栏位置相同），尾部的"悬崖深度"却天差地别：一个突破后平均只多亏一点，另一个突破后会血本无归。CVaR 把这条被 VaR 刻意回避的"深渊深度"重新纳入视野，这就是它在风险管理上更诚实的根本原因。

CVaR 还有一个理论优势——它是**一致性风险度量（coherent）**，满足**次可加性**（组合的风险不超过各部分之和），而 VaR 不满足。次可加性翻译成大白话就是"**分散化不应该让风险变大**"——把两个资产放进同一个组合，整体风险至多等于各自风险之和、通常还更小，这是"不要把鸡蛋放一个篮子"这条金融第一直觉的数学表达。可 VaR 在某些情形下竟会**违反**这条常识：两个资产合并后的 VaR 反而大于二者 VaR 之和，等于在说"分散化让你更危险了"——这显然荒谬，会诱导出"拆分持仓来粉饰风险"的扭曲行为。CVaR 没有这个毛病，它对分散化的奖励始终如一。正因为既能看清尾部深渊、又在数学上自洽，巴塞尔新规（FRTB）已从 VaR 转向 **ES** 作为市场风险资本的基础——这是监管层用真金白银的资本要求，为"度量风险要老实面对尾部"投下的一票。
