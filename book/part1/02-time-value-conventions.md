...
!!! note "本章知识结构"
    **现值 / 终值 / 折现**（2.2，定价的基石）→ **年金 / 等额本息**（2.3）
    　↓ 利息 = 本金 × 利率 × 计息时间
    **计息惯例**（2.4，ACT/ACT、ACT/365、ACT/360、30/360）
    　↓ 成交价的两个口径
    **净价 / 全价 / 应计利息**（2.5）→ `fi.cashflow` 与 QuantLib `DayCounter` 实现（2.6–2.7）
...
在套用公式之前，有必要先厘清一个更基本的问题：**为什么"今天的 1 元"应当比"明年的 1 元"值钱？** 这并非金融学的武断假设，而是三重因素共同作用的结果。其一是**机会成本**：今天的 1 元可用于存款、购债或投资，一年后将增值为 1 元有余；因此，延迟一年才取得的 1 元，相当于放弃了这段时间本可获得的利息。其二是**通货膨胀**：物价通常逐年上涨，明年的 1 元所能购买的商品往往少于今天的 1 元，购买力随时间递减。其三是**不确定性**："明年能够取得"终究只是一项承诺，对手方可能违约、外部环境可能变化，而当下持有的资金则是确定的。三者共同决定了：未来的现金必须经过折算，方能与当前的现金置于同一标准下加以比较。**折现率 $r$ 即市场为"等待"所确定的价格**——它将机会成本、通胀预期与风险补偿一并纳入了折现因子的分母之中。
...
这一公式背后还隐含着一个值得注意的结构性特征。等额本息的月供数额始终恒定，表面上呈现为"每月均匀还款"；然而，若将每笔月供拆分为"利息"与"本金"两部分，情形便有所不同：在还款的**前若干年，月供中绝大部分用于支付利息，真正偿还本金的比例很低**。究其原因，利息按"当期剩余本金"计算，而期初剩余本金最大，故利息占据了月供的主要部分；随着本金逐步偿还，利息部分才逐月减小、本金部分才逐月增大。这也解释了为何借款人在还款数年之后会发现本金减少有限，以及为何**等额本息方式下提前还款越早越为有利**（早期所节省的利息最多）。图2-1 直观地呈现了这条"利息逐月让位于本金"的曲线，它不仅是一道计算问题，也刻画了长期等额本息贷款现金流的内在构成。
...
!!! note "知识点自测清单"
    - [ ] 默写并计算**现值 / 终值 / 年金**公式
    - [ ] 解释为什么比较两个利率必须说清**复利频率**
    - [ ] 用年金公式算**等额本息月供**
    - [ ] 说出 ACT/ACT、ACT/365、ACT/360、30/360 各自的算法
    - [ ] 解释 ACT/ACT 与 ACT/365 何时相同、何时（跨闰年）不同
    - [ ] 计算给定结算日的**应计利息**与全价
    - [ ] 解释"净价报价、全价结算"
