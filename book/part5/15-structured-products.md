...
前面我们给单只债券定价。本章换一种思路：把**一大堆资产**（房贷、车贷、应收账款……）的现金流**打包、切割、再分配**，造出新的证券——这就是**资产证券化（securitization）**。其核心机制是**分层（tranching）**：同一个资产池，切成"优先—夹层—次级"几档，损失自下而上由次级先吸收，于是优先档获得了远高于资产池本身的信用质量。

这套技术既是金融工程的精华，也是 2008 年全球金融危机的导火索——当底层资产（次级房贷）的违约相关性被低估，"安全"的优先档同样崩塌。理解结构化产品，既要懂它如何**创造**信用质量，也要警惕它如何**隐藏**风险。本章把第14章的违约/回收概念用到**分档定价**上。
...
!!! note "本章知识结构"
    **证券化原理**（13.2，SPV / 真实出售 / 破产隔离，信用脱离发起人）
    　↓ 用次级垫底创造信用
    **分层结构**（13.3，优先/夹层/次级、附着脱离点）
    　↓ 现金按优先级下流
    **现金流瀑布 + 触发机制**（13.4）→ **信用增级**（13.5，内部/外部）
    　↓ 利率敏感的现金流
    **MBS 提前偿还**（13.6，收缩/延展风险、负凸性）→ 中国 ABS/RMBS（13.8）
...
这里发生的事情，看似将低资质资产转化为高评级证券：底层是一池信用一般、违约率不低的资产（如次级房贷、消费贷），经过分层这道工序，却能切出一块评级 AAA、堪比国债的优先档。资质平平的原料之所以能产出顶级证券，关键在于"谁先承担损失"的顺序安排。可将资产池的损失类比为从楼底向上注入的水：损失先淹没位于最底层的次级档，次级被淹没后才及于中间的夹层，夹层也满后才漫至顶层的优先档。因此，只要损失水位不超过次级加夹层的高度，位于顶层的优先档便不受触及。换言之，优先档的 AAA 评级并非凭空产生，而是由次级档先行吸收损失的安排换来的。次级投资者明知自己最先承担损失却仍愿购买，是因为他们处于瀑布的末端：一旦资产池表现良好、损失未及其所在层级，剩余的超额收益便全部归其所有，属于高风险高回报的配置。由此可见，分层的本质是把同一池现金流按损失吸收的先后顺序重新切割，**使偏好安全的投资者购买优先档、偏好博取收益的投资者购买次级档，各取所需**。但须强调：分层只是重新分配了损失，**它并未减少资产池的总损失**——这正是 13.5 节 2008 年教训的伏笔。
...
!!! example "例13.1：分层与损失分配"
    ...
    **结论**：只要池损失 ≤ 20%（次级 5 + 夹层 15 的厚度），**优先档零损失**——这正是分层得以将优先档提升至 AAA 评级的机制。

<figure markdown>
  ![图15-1　分层与次级垫底](../assets/figures/ch15_tranching.png){ width="640" }
  <figcaption>图15-1　各档损失率随资产池损失率变化：优先档在池损失突破 20%（其附着点）前零损失，次级/夹层先行垫底</figcaption>
</figure>
...
提前还款常用 **PSA 模型**（提前还款速度的标准曲线）刻画。MBS 的这种"利率一动、现金流就变"的特性，使其分析与第9章可赎回债一脉相承——都是嵌入了借款人的"提前偿还期权"。

理清这层关系，MBS 的风险结构便清晰可辨：**每一个房贷借款人都持有一份提前还款期权**，而 MBS 投资者即为这份期权的卖方。这与第9章可赎回债中"投资者向发行人卖出赎回权"属于同一机制，只是行权方由企业变为众多购房者。利率下行时，借款人如同可赎回债的发行人一般行权——将高息旧贷提前结清、再融入利率更低的新贷，本金随之提前回流至 MBS 投资者，而投资者只能将这笔资金再投资于当下利率更低的资产（**收缩风险**）。利率上行时，借款人则倾向于延后偿还，MBS 的久期被动拉长，恰逢利率上行、最不利于久期延长之时（**延展风险**）。两种情形均对投资者不利——这正是第6章所述"负凸性"的再现：利率下行时价格上行空间受限（本金被提前偿还），利率上行时久期又难以缩短（提前还款放缓）。由此可见，MBS 投资者所获的额外利差，与可赎回债的较高票息同理，本质上是为卖出这份借款人期权所收取的权利金。
...
1. **损失分配与附着点**（图15-1）：画出各档损失率随资产池损失率的阶梯曲线，定位优先档的"安全垫"；
...
7. 用 `fi.securitization` 复现例13.1，并复现图15-1（各档损失率 vs 资产池损失率的阶梯图）。
...
!!! success "概念题 2"
    **次级垫底**：损失自下而上分配，次级档先吸收，优先档最后受损。用附着/脱离点：次级 0–10%、夹层 10–30%、优先 30–100%（计算题 5 的结构），则**只要资产池损失 ≤ 30%（次级+夹层厚度），优先档零损失**——优先档的"附着点"高达 30%，相当于有 30% 的安全垫，因此能获得远高于资产池整体的评级。

!!! success "概念题 3"
    **内部增级**：① 次级垫底（subordination）；② 超额抵押（资产 > 证券面值）；③ 超额利差（资产利息 > 证券利息+费用）；④ 储备账户。**外部增级**：① 第三方担保；② 保险/信用证。外部增级引入了**担保方的信用风险**——2008 年单线保险商（monoline）崩塌就是例证：担保方一倒，被担保证券同步失去保护。

!!! success "概念题 4"
    分层只是**重新分配**损失，不减少资产池**总损失**。2008 年次贷被层层打包成 CDO、CDO²，制造大量"AAA"；当房价普跌、违约**高度相关**地集中爆发，损失远超次级+夹层厚度，"安全"的优先档同样巨亏。**违约相关性是关键**：分层的保护建立在"违约分散、不会同时发生"的假设上；一旦相关性飙升（系统性事件），再厚的次级也被击穿。

!!! success "计算题 5"
    资产池 100，优先 70/夹层 20/次级 10。**附着/脱离点**（自下而上累积占比）：次级 **0–10%**、夹层 **10–30%**、优先 **30–100%**。损失分配：

    | 资产池损失 | 次级 | 夹层 | 优先 |
    |---|---|---|---|
    | 15% | 10（全损） | 5（5/20=25%） | 0 |
    | 35% | 10（全损） | 20（全损） | 5（5/70=7.1%） |

    损失 15% 时优先档零损失（未突破 30% 附着点）；损失 35% 时优先档才承担 5（突破 30% 后的部分）。

!!! success "计算题 6"
    MBS 借款人会**提前还款**：利率下行时再融资、提前结清房贷，本金提前回笼，投资者只能以更低利率再投资——这是**收缩风险**。它与**可赎回债的负凸性相似**：都是"利率下行→借款人/发行人行使提前偿还/赎回的期权→投资者价格上行空间被封顶"。MBS 相当于内嵌了借款人的"提前偿还期权"，故同样呈现负凸性。

!!! success "编程实验 7–9（要点）"
    - **实验 7**：`fi.securitization` 复现例13.1；图15-1 阶梯图——次级先打穿（0–5%）、再夹层（5–20%）、优先在 20% 后才受损。
    - **实验 8**：`sequential_waterfall` 在违约率 0/10/20/30% 下，资产池损失 = 违约率 ×LGD× 池规模，优先档先足额受偿、次级先承担损失；画各档回收率曲线可见次级最先归零。
    - **实验 9**：给资产池现金流加入提前还款（按月削减/加速本金），各档久期缩短、现金流提前——直观感受 MBS 的收缩风险。
...
下一部分（第16章起）进入**市场机制和综合应用**：货币市场工具与回购、债券投资策略与回测，以及把全书工具整合为完整框架的风险管理系统。

!!! note "知识点自测清单"
    - [ ] 解释**真实出售 / 破产隔离**为何让 ABS 信用脱离发起人
    - [ ] 用**附着/脱离点**说明次级垫底如何让优先档获得高于资产池的评级
    - [ ] 计算给定资产池损失下各档（优先/夹层/次级）的损失分配
    - [ ] 描述**现金流瀑布**（利息瀑布 + 顺序还本）与触发机制
    - [ ] 列举内部/外部**信用增级**方式及外部增级的新风险
    - [ ] 用 2008 危机解释"分层不消灭风险"、违约相关性为何关键
    - [ ] 解释 MBS **提前偿还**（收缩/延展风险、负凸性）
