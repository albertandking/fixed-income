...
一种常见的误读，是把信用利差等同于"违约的赔率"——即认为利差仅由市场预期的违约概率所决定。这一理解只覆盖了利差的一部分。信用利差实由三层补偿叠加而成：**第一层**是预期违约损失，即"违约概率 × 违约损失率"的精算期望，这是利差中唯一能由历史违约数据解释的部分。**第二层**是风险溢价——违约并非平稳发生的随机事件，它往往在经济衰退时集中爆发，而衰退恰是投资者资金最为紧张的时点；因此，投资者会为承担"最坏时点的不确定性"额外索取补偿，即便长期平均违约损失并不高。**第三层**是流动性溢价——信用债的可变现性远低于国债，急需资金时可能不得不折价出售，这份"难以及时变现的风险"同样需要计价。值得注意的是，对高评级债而言，真正的预期违约损失往往只占利差的**一小部分**，其余主要来自后两层补偿——这便为 12.5 节"信用利差之谜"埋下了伏笔。由此可见，"利差走阔"未必意味着"市场预期违约临近"：许多情形下，走阔反映的是风险厌恶上升与流动性紧张，而非违约预期本身的变化。
...
Merton（1974）的关键贡献，是把公司**资本结构**与**期权**联系起来。设公司资产价值 $V$ 服从几何布朗运动，负债是一笔到期 $T$、面值 $D$ 的零息债。到期时：
...
这一洞见的关键之处，在于它把会计中"资产 = 负债 + 所有者权益"这一静态恒等式，转译为期权定价的动态语言，而其枢纽正是**股东的有限责任**。当公司经营良好、资产远超负债时，股东可取得全部剩余，收益上不封顶；而一旦资产跌破负债、资不抵债，股东也只以有限责任为限承担损失，亏损被锁定在已投入的股本之内，不会进一步扩大。这种"收益无上限、亏损有下限"的损益结构，与买入一份看涨期权完全一致：行权价为负债 $D$，标的为公司资产 $V$。沿此思路，Merton 进一步刻画了整个公司资本结构：股东持有看涨期权，则**债权人相当于卖出了一份看跌期权**——平时收取固定利息（相当于权利金），而一旦公司陷入违约（资产跌破负债），便须被动承接缩水的资产并承担损失。由此，"公司是否会违约"这一原本需依赖基本面研究与管理层访谈才能判断的定性问题，被转化为"这份看涨期权是否会到期作废"的**可计算**问题——只要给定资产价值、资产波动率与负债水平，违约概率即可由 Black-Scholes 的同一套公式求得。这正是结构化模型的巧妙之处：**它使违约第一次拥有了定价公式。**
