...
第6章讨论了如何**度量**一只债券的利率风险（久期与凸性），本章则进一步讨论如何**管理**：当所持有的不是孤立的一只债券，而是要让**一整个资产组合**去对冲**一组未来负债**时，应当如何配置？这正是保险公司、养老金、银行资产负债管理（ALM）日常面对的核心问题——**负债驱动投资（Liability-Driven Investing, LDI）**。

本章的主线是一个重要洞察：第4章已经指出，利率下行时**再投资收益**下降（不利）而**债券价格**上升（有利），两种风险方向相反。若能使二者**恰好抵消**，便可把未来某个时点的财富**锁定**下来，不受利率波动影响——这就是**免疫（immunization）**。本章将这一洞察由直觉发展为可计算、可求解的策略。
...
!!! note "本章知识结构"
    **负债驱动投资 + 免疫思想**（7.2，价格风险与再投资风险对冲）
    　↓ 三个条件
    **Redington 免疫**（现值匹配 + 久期匹配 + 凸性占优）
    　↓ 两条实现路线
    **久期匹配 / 单期免疫**（7.3，哑铃 vs 子弹、需再平衡）｜ **现金流匹配**（7.4，LP、无需再平衡）
    　↓
    `fi.portfolio` 实现（7.5）→ 负债驱动组合案例（7.7）

## 7.2 负债驱动投资与免疫思想

### 7.2.1 从"追求收益"到"对冲负债"

主动投资追求超额收益，但许多机构的首要目标并非"收益最大化"，而是"**到期能够如约兑付**"：保险公司须在未来若干年兑付保单，养老金须按月发放退休金。对这类机构而言，风险的定义不是收益波动，而是**资产不足以覆盖负债**。负债驱动投资（LDI）因此把负债当作"基准"，让资产去**匹配**其现值与利率敏感度。这意味着考核视角的根本转变：一般基金经理关注的是相对市场基准的超额收益，而 LDI 经理关注的是资产久期与负债久期是否对齐——对后者而言，主要风险不是收益落后，而是**资产与负债对利率的敏感度不一致所形成的缺口**。

若处理不当，其后果可能具有系统性。2022 年秋，英国国债收益率因一份扩张性的"迷你预算"在数日内大幅上行，使当地养老金普遍采用的 **LDI 策略**承受了严重压力：这些基金为以有限资金匹配超长期负债的久期，较多使用了**加杠杆的利率衍生品**；利率快速上行时，衍生品头寸随即产生大额追加保证金要求，基金被迫**抛售国债以补充流动性**，而抛售又进一步推高收益率、引发更多保证金催缴，形成自我强化的负反馈。最终由英格兰银行临时入场购债加以稳定。这一事件表明：**匹配久期的方向是恰当的，但以杠杆方式实现匹配，可能将利率波动放大为流动性风险**。就中国市场而言，寿险公司则长期面临另一种 LDI 困境——**资产久期短于负债久期**（市场缺乏足够长期的资产去匹配数十年的保单负债），在利率持续下行时，到期资产只能以更低收益率再投资，由此积累"利差损"隐患。无论是英国的杠杆负反馈，还是中国的久期缺口，根源均在于**资产与负债的利率敏感度未能对齐**。本章所述的免疫，正是实现这一对齐的基本方法。
...
免疫之所以恰好在"久期=持有期"处实现，可由数学视角加以说明。把 $H$ 时点的终值写成 $W(y)=P(y)\times(1+y)^{H}$——前一项是债券在 $H$ 时的价格（随 $y$ 上升而下降），后一项是将早期票息复利累积至 $H$ 的再投资因子（随 $y$ 上升而上升）。要使 $W$ 对利率不敏感，令 $\frac{dW}{dy}=0$；代入价格对 $y$ 的导数正比于 $-D\cdot P$（修正久期的定义），整理后使两项相反效应恰好平衡的条件，**正是麦考利久期 $=H$**。可见"久期=持有期"并非需要记忆的结论，而是"价格下跌的速率"与"再投资上升的速率"在 $H$ 点相等时，数学上必然得到的位置。换言之，再从**经济视角**看：久期短于 $H$ 时，组合变现偏早，再投资风险占主导，对利率下行较为敏感；久期长于 $H$ 时，组合期限偏长，价格风险占主导，对利率上行较为敏感；**唯有久期恰好等于 $H$ 时，组合在该时点对利率的两个变动方向都不敏感**。

!!! tip "一个机械的类比"
    可将价格部分与再投资部分视为跷跷板的两端：利率变动时，一端上升、一端下降。久期相当于支点的位置——当支点位于 $H$ 处（久期=持有期）时，跷跷板在 $H$ 点保持水平，无论利率如何波动，该点的高度（财富）几乎不变。
...
前两条保证盈余对利率的一阶导数为零；第三条保证二阶导数非负——于是利率**无论往哪个方向**小幅移动，盈余都**不减**。由此可见一个常被忽略的要点：**免疫不仅要匹配久期，还要使资产凸性不低于负债凸性**（其原因留作习题分析）。
...
!!! example "例7.1：单期免疫的数值演示"
    ...
    **结论**：实现终值在 $\Delta y=0$ 处取得**最小值**，任何方向的利率冲击都使终值**略高于**目标——盈余得到免疫，且因正凸性而呈现"涨幅大于跌幅"的特征。这正是免疫的作用：将 $H$ 时点的财富稳定在目标水平之上。
...
久期会**随时间推移和利率变动而漂移**：时间经过一年，负债久期下降约 1，而资产久期通常下降不足 1；利率变动时，久期亦随之改变。因此免疫组合须**定期再平衡（rebalance）**以重新对齐久期——这构成免疫的隐性成本（交易费用、跟踪误差），也是它与下文"现金流匹配"的关键区别。
...
**现金流匹配（cash flow matching / dedication）**采取另一条路径：不依赖久期近似，而是**使资产在每个时点产生的现金流，恰好覆盖该时点的负债**。若每一期均实现现金流匹配，则无论利率如何变动，到期都有相应资金可供兑付——**无需再平衡，也不存在再投资风险**。

这一思路与免疫存在本质区别。免疫是一种**统计意义上的对冲**：它借助久期这一"平均量"使财富对利率变动实现一阶免疫，但需持续监测并不断再平衡，本质上是以动态调整换取灵活性与较低成本。现金流匹配则是一种**逐期精确对应**：若第 3 年需兑付 100，便配置一只在第 3 年恰好回款 100 的债券，使资产现金流与负债逐项一一对应。在此安排下，利率的后续变动不再产生影响——因为无需在中途卖出资产（不存在价格风险），也无需将现金再投资（不存在再投资风险），两类风险被从来源上消除，而非如免疫那样使其"相互抵消"。正因如此，现金流匹配在"金额不容偏差、且负债时点完全确定"的场景中具有不可替代性，例如法院判决的**结构化赔付**、到期日锁定的**专项养老金**。其代价在于约束较强——可选债券须在相应期限上具有相应的现金流，约束增多则成本通常高于较为灵活的免疫。**免疫是将风险"对冲消除"，现金流匹配则是将风险"从根本上规避"**，这是理解 7.4.3 取舍表的总纲。
...
!!! success "概念题 2"
    Redington 三条件中，现值匹配 + 久期匹配保证盈余（资产−负债）对利率的**一阶导数为零**；**凸性占优（资产凸性 ≥ 负债凸性）**保证**二阶导数非负**，于是利率无论上行下行，盈余都**不减**（凸性给"涨多跌少"的保护）。若资产凸性更低，则大幅利率变动时盈余可能转负——免疫被破坏。

!!! success "概念题 3"
    哑铃（两端期限）与子弹（集中期限）久期可相同，但**哑铃现金流更分散、凸性更高**（凸性按 $t^2$ 量级加权）。凸性越高，对大幅利率变动越稳健（涨多跌少更明显），故哑铃免疫更稳健。**代价**：哑铃通常收益率略低（为凸性付费），且对**曲线非平行移动**（变陡/变平）更敏感（两端暴露）。

!!! success "概念题 4"
    **免疫需再平衡**：久期随时间流逝和利率变动而漂移（时间过一年，负债久期降约 1，资产久期降不足 1），需定期调整以重新对齐久期。**现金流匹配不需要**：它让每期资产现金流精确覆盖负债，无论利率怎么变都有钱可付，是"一劳永逸"的（代价是成本更高、约束更强）。

!!! success "计算题 5"
    哑铃权重解 $w_s D_s + w_l D_l = 6$、$w_s+w_l=1$：

    - $D=2\,\&\,9$：$w_l=(6-2)/(9-2)=0.5714$、$w_s=0.4286$；
    - $D=4\,\&\,7$：$w_l=(6-4)/(7-4)=0.6667$、$w_s=0.3333$。

    **$D=2\,\&\,9$ 组合凸性更高**——两端期限跨度更大、现金流更分散，$t^2$ 加权的凸性更大。这印证"越哑铃、凸性越高"。

!!! success "计算题 6"
    自后向前匹配。**第 2 年**只有 2 年期债付款（每份 1.06）：需 $1.06\,F_2=150\Rightarrow F_2=141.51$。**第 1 年**：2 年期债贡献 $0.06\times141.51=8.49$，缺口 $50-8.49=41.51$ 由 1 年期零息（每份付 1）补：$F_1=41.51$。**总成本**：

    $$141.51\times1.00+41.51\times0.98=\mathbf{182.19}$$

    （用 `fi.portfolio.cash_flow_match([50,150], [[1,0],[0.06,1.06]], [0.98,1.00])` 验证，最小成本 182.19，各期精确覆盖。）

!!! success "编程实验 7–9（要点）"
    - **实验 7**：`fi.portfolio` 复现例7.1，画"实现终值 vs Δy"——U 形、在 Δy=0 处取最小（图7-1），任何方向利率变动终值都 ≥ 目标（正凸性）。
    - **实验 8**：`cash_flow_match` 复现例7.3（成本 278.905）；增删可选债会改变最小成本与组合构成——可选券越多，匹配越灵活、成本可能越低。
    - **实验 9**：构久期=7 哑铃，"时间+1 年 & 利率+50bp"后重算久期，会发现久期漂移（不再恰好=7），据此求再平衡调整量——这量化了免疫的隐性成本。
...
下一部分（第8章起）进入**利率衍生品和高级品种**：先从浮动利率债券讲起，继而是可赎回、可转换等含权债券，再到利率期货、互换与期权；其中含权债的负凸性会让本章的"凸性占优"条件变得更微妙。

!!! note "关键公式速查"
    | 概念 | 公式 |
    |---|---|
    | 组合久期 | $D_P=\sum_i w_i D_i$（市值加权） |
    | 单期免疫 | 久期 = 持有期 $H$ |
    | 两券（哑铃）权重 | $w_l=\dfrac{D_{target}-D_s}{D_l-D_s}$，$w_s=1-w_l$ |
    | Redington 三条件 | 现值匹配 + 久期匹配 + 凸性占优（$C_{资产}\ge C_{负债}$） |
    | 现金流匹配（LP） | $\min_x p^\top x$ s.t. $C^\top x\ge \ell,\ x\ge0$ |
