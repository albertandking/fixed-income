...
久期不是抽象的数学游戏，它的威力（与代价）在真实市场里反复上演。2022 年 11 月，中国债市经历了一轮剧烈调整：利率快速上行，那些持有**长久期**债券的银行理财产品净值大跌，触发投资者赎回，理财被迫抛债，债券进一步下跌又压低净值、引发更多赎回——一场"下跌→赎回→抛售→再下跌"的负反馈螺旋席卷市场。事后复盘，受伤最重的，正是久期最长的那批产品。**久期有多长，利率上行时就亏多少**——这条朴素的规律，本章要把它讲透、算准，并教你如何对冲。

这个概念有近百年历史。1938 年，加拿大经济学家 **Frederick Macaulay** 在研究美国铁路债券时，提出用"现金流的现值加权平均回收时间"来刻画债券的有效期限——这就是今天的**麦考利久期**。此后 Hicks 从弹性角度、Redington 从负债免疫角度（第7章）、Fisher 与 Weil 从期限结构角度不断拓展，久期才从一个朴素的"期限度量"，演变成现代固定收益的核心**利率敏感度度量**。一个看似简单的加权平均，背后是几代人的打磨。
...
先想象一个具体场景。你管理着一个上亿元的债券组合，风控总监走过来问："如果明天央行加息 25 个基点，我们大概亏多少？"你当然可以把组合里几百只债券逐一按新收益率重新定价、加总、再减去原值——但这既慢又笨，而且无法**事先**为风险设限额。我们真正想要的，是一个**预先算好的数字**，让你一眼估出"利率每动 1 个基点，价值变动多少"。这个数字，就是久期。
...
到这里，久期已经长出了**四张互相印证的面孔**，值得停下来细品——真正用得顺手的人，脑子里同时装着这四个角度：

- **几何的脸**：久期是价格—收益率曲线在当前点的**切线斜率**（标准化后）。利率动一点，价格就沿这条切线滑动 $-D_{\text{mod}}$ 个百分点。后面讲凸性，就是说"真实曲线是弯的、切线会偏"。
- **数学的脸**：久期是价格对收益率的**一阶弹性**——把非线性的定价公式在当前点做线性化，得到的那个灵敏度系数。
- **时间的脸**：麦考利久期是现金流的**现值加权平均回收时间**，回答"我投出去的钱，平均多久收回来"。
- **经济的脸**：久期是利率风险的**统一标尺**。10 年期国债久期约 8.5、1 年期约 1，于是同样加息 100bp，前者市值缩水约 8.5%、后者只缩水约 1%——久期把"谁更怕加息"压成了一个可横向比较的数字。

这四张脸说的是同一件事，只是看的角度不同。初学时人们往往只背下公式（数学的脸），但在交易台和风控室里，**几何（切线）与经济（标尺）的直觉往往更顶用**——它们让你在没有计算器时也能对风险有感觉。
...
这张表背后是同一条直觉：**久期取决于"现金流的重心落在多远"**。任何把现金流重心往前拉的因素，都会缩短久期。票息率就是最典型的例子——高票息债每年还回更多现金，重心前移，所以同样 10 年期，一只票息 5% 的债久期会明显短于票息 2% 的债。这条规律有个重要的实务推论：**在利率下行预期里想拉长久期博价格上涨，应该买低票息、长期限的债**（极端情形就是长期零息债，久期=期限、弹性最大）；反过来，想降低利率敏感度、又不想缩短到期期限，可以换成高票息债。

收益率水平的影响则容易被忽略：**收益率越高，久期越短**。原因是高收益率把远端现金流折得更狠，远端权重下降、重心前移。这意味着同一只债，在 2% 的低利率环境和 6% 的高利率环境下，久期并不相同——低利率时久期更长、更"危险"。这也部分解释了为什么近年低利率环境下，债券组合对加息格外敏感：利率越低，久期越长，加息的杀伤力越大。
...
修正久期有一个隐藏的前提：现金流是**固定**的。正因为现金流不随利率变，我们才能对那条光滑的定价公式求导。可现实里有一大类债券不满足这个前提——它们的现金流会**随利率自己改变**。可赎回债在利率下行时被发行人提前赎回（第10章）、MBS 的房贷借款人在利率下行时提前还款再融资（第13章）、可转债在股价上涨时被转股（第11章）。对这些债券，"价格—收益率曲线"不再光滑可导，甚至在某些利率区间会**反向弯折**（负凸性），解析的修正久期公式直接失效。

那怎么办？退回到导数最原始的定义——**差商**。我们不再求解析导数，而是"亲手动一下利率、看价格变多少"：把整条收益率曲线向上、向下各平移一个小幅度 $\Delta y$，**各自重新定价**（含权部分让定价模型重新判断是否行权），再相除。这就是**有效久期**：
...
这在实务里至关重要。债券基金经理常做**曲线交易**：预期收益率曲线**变陡**（长端相对短端上行更多）时，减持长端、增持短端，或把"子弹"换成"哑铃"；预期**变平**时反向操作。这类策略的损益完全由曲线**形状**变化驱动，而组合的总久期可能纹丝不动——一个总久期为 5 的组合，既可能把敞口堆在 10Y，也可能分散在 2Y/5Y，二者在曲线变陡时的盈亏天差地别，但总久期给出的是同一个数字。一句话：**总久期管的是"利率涨不涨"，关键利率久期管的是"曲线怎么扭"**。中国债市活跃的"做陡/做平"策略，本质就是在交易 KRD 的分布。
...
这不是纸上谈兵，而是市场上真金白银交易的东西。成熟市场的交易员会专门**买入凸性**——用期权、或用现金流分散的哑铃组合，构造高凸性头寸，押注利率大幅波动（无论涨跌都占便宜）；而**卖出凸性**的一方（如卖出含权债、做空利率期权）收取权利金，赌的是利率平稳。**凸性是一种可以定价、可以买卖的风险**。这也解释了一个常见现象：两只久期相同的债，凸性高的那只收益率反而略低——市场让你为"涨多跌少"的好处付费。

硬币的另一面是**负凸性**。MBS 和可赎回债的持有人被动持有了负凸性：利率大跌时，别人的债大涨，他们的债却因为提前偿还/赎回而被"封顶"，涨不上去。买入这类资产，等于卖出了凸性、收了一点额外票息，代价是放弃了大波动里的上行——这正是第10章（可赎回债）与第13章（MBS）的核心痛点。理解了凸性是有价的，就理解了为什么含权债要给更高的收益率。
...
走进任何一家银行或券商的交易室，你听到的不会是"这只债久期 8.5"，而是"这个簿子 DV01 五百万"——意思是利率动 1bp、这个交易簿的盈亏就是 500 万元。**DV01 是交易台的母语**。风控按 DV01 给每个交易员、每个交易簿设限额（如"国债簿 DV01 不得超过 800 万"），既直观、又能层层汇总：把所有持仓的 DV01 一加，就是整个机构的利率总敞口；想知道某笔交易增加了多少风险，看它的 DV01 就够了。久期是百分比、不能跨券相加，DV01 是金额、可以从单券一路汇总到全行——这就是实务中 DV01 比久期更常用的根本原因（详见下方提示框）。
...
这套对冲在实务中天天发生，但有三点务必清醒。**第一，它只对冲一阶风险。** DV01 中性让组合对小幅平行移动的损益归零，但凸性差异仍在——目标券与对冲工具的凸性若不同，大波动下两边并不完全抵消，会留下二阶残差。**第二，它假设曲线平行移动。** 若曲线变陡变平（短端长端动得不一样），用一只工具债对冲一个跨期限组合，照样会漏风——这时要按关键利率久期分段对冲（6.3.2）。**第三，对冲比率会漂移。** 利率一变、时间一过，两边的 DV01 都在变，今天的中性到明天就不中性了，必须**动态再对冲（dynamic rehedging）**——再对冲越频繁、跟踪越紧，但交易成本越高，这是一对永恒的权衡。

举个具体场景：一家保险公司持有大量长久期国债，担心加息，于是卖出国债期货把组合 DV01 砍掉一半。这一步用的就是上面的公式（第14章会算出确切手数）。但它知道，期货盯住的是"最便宜可交割券"、其久期会随利率跳变（CTD 切换），凸性也和现券不同——所以这是一个需要持续盯盘、定期调整的"活"对冲，而不是一劳永逸的"死"保险。**完美对冲在现实中并不存在，理解残差在哪里，比追求账面上的"零敞口"更重要。**
...
这四条结论串起来，正是一个组合经理拿到风险报告后的完整思考链：先看**总久期**知道"利率动一下大概亏多少"，再看 **DV01 的分布**知道"风险压在曲线哪一段"，接着用**凸性**修正"一阶估计偏乐观还是偏保守"，最后据此**决定要不要对冲、对冲多少"**。久期不是一个孤立的数字，而是把"利率会怎么动"翻译成"我的组合会怎么变"的一整套语言——本章前面所有的公式（修正久期、KRD、DV01、凸性、对冲比率），到这里第一次合奏成一支完整的曲子。

尤其值得在意的是第 2 条揭示的"敞口集中"。组合久期 5.14 听上去温和，但 57% 的利率风险压在 10Y 这一个点上——如果未来曲线"熊陡"（长端比短端涨得多），这个组合受的伤会比"久期 5.14"这个平均数暗示的更重。这正是为什么风控不能只盯一个总久期数字、而要看 KRD 的分布：**平均数会骗人，分布不会**。配套 notebook 进一步画出该组合的**价格—收益率曲线**与**各券 KRD 柱状图**，把这种"局部线性近似"与"敞口集中在长端"的事实直观地摆在眼前。
...
回到本章开头那个问题——"利率动一下，组合亏多少？"我们用一条主线给出了越来越精细的答案：先对价格求**一阶导数**，得到**久期**，回答"涨跌幅大约多少"；发现曲线是弯的，便补上**二阶导数**——**凸性**，把大波动下的误差抹平，并意外发现它对投资者有利、还能交易；意识到现金流可能不固定、曲线可能不平行移动，于是引入**有效久期**与**关键利率久期**；最后把百分比敏感度换算成金额——**DV01**，让风险可限额、可对冲、可汇总到全行。一阶到二阶、单点到全曲线、百分比到金额，这就是利率风险度量的完整阶梯。把这套工具握在手里，第7章的免疫、第14章的期货套保、第18章的 VaR 都只是它的延伸。要点回顾：
