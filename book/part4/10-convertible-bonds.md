...
<figure markdown>
  ![图10-1　可转债的股债性切换](../assets/figures/ch10_convertible.png){ width="660" }
...
6. 用 `fi.convertible` 复现例11.1、例11.2，并复现图10-1（可转债/纯债底/转股价值三条线），标出股性区与债性区。
...
!!! success "编程实验 6–8（要点）"
    - **实验 6**：`fi.convertible` 复现例11.1（S=5/8/12 → 价 98/112/140）、例11.2；图10-1 三条线（可转债曲线在纯债底与转股价值之上），低股价为债性区、高股价为股性区。
