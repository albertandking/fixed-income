...
到目前为止，本书所讨论债券的现金流都是确定的。但现实中大量债券**带有嵌入期权**：发行人可以提前赎回（**可赎回债**），或投资者可以提前回售（**可回售债**）。这些期权使现金流**随利率变化**——利率下行时发行人赎回旧债以低成本再融资，利率上行时投资者回售以拿回本金。一旦现金流不再确定，前述"逐笔折现"的定价方法即**不再适用**，必须引入能够刻画"利率沿不同路径演化、期权相机行权"的新工具：**利率树**。

更重要的是，嵌入期权会带来第6章未曾出现的现象——**负凸性**：可赎回债在低利率区价格被"封顶"，难以继续上行。这使久期、凸性的分析变得微妙，也催生了一个专门的利差指标——**期权调整利差（OAS）**。本章是全书技术性较强的一章，它首次将"期权"与"债券"这两条主线结合在一起。
...
这一机制可类比为**房贷的提前还款**。借款人当初以 5% 签下房贷，若干年后市场利率降至 3%，便可提前结清旧贷、再借一笔成本更低的新贷，相当于将高息债务"赎回"。可赎回债的发行人面临的情形与此相同：当初以 6% 高息发债，利率下行后行使赎回权，以赎回价收回旧债，再以更低成本发新债。换言之，赎回权对发行人构成一份再融资期权——利率走势对其有利时行权，不利时则不行权。这份权利并非无偿取得，而是投资者以让渡更高利息（更高的票面或收益率）作为权利金卖出的。可赎回债之所以收益率更高，正是因为投资者须承担"债券可能在利率下行、价格升值时被强制赎回"这一风险，并就此收取对价。由此可见，把握"期权归发行人、权利金归投资者"这一关系，是理解含权债定价逻辑的基础。
...
考虑一个常见的实务问题：同一家公司发行两只债，一只普通债收益率 4%，一只可赎回债收益率 5%。后者收益率高出 100bp，是否意味着更具吸引力？此处不宜直接下结论。这多出的 100bp 中，一部分是真正的信用与流动性补偿，另一部分则只是投资者卖出赎回权所收的权利金，即为承担被赎回风险付出的对价，并非额外的无偿收益。值得注意的是，将两只债的收益率直接相减，等于以一个"净"口径的数字去比较一个"含期权"的数字，二者并不可比。OAS 所要解决的正是这一可比性问题：它将收益率中归属于期权的部分剔除，只保留纯粹的信用补偿，从而使含权债与不含权债能够在同一口径下比较。这正是其名称中"期权调整（option-adjusted）"的由来，也是相对价值分析须依赖该指标的根本原因。
...
因此，讨论 OAS 时必须明确波动率假设；不同机构所用波动率模型不同，算得的 OAS 也会存在差异。这一点亦由本章编程实验加以验证。
