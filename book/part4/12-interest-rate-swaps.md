...
互换的价值在于：无需买卖现券，仅凭一纸合约即可将"浮动利率敞口"转换为"固定"（或反向操作）。有浮动利率负债的企业，签订一笔"付固定、收浮动"的互换，便可将负债锁定为固定成本。由此可见，互换是利率风险管理、利率观点表达与资产负债管理的通用工具。它的定价完全建立在第5章的折现曲线之上——理解了曲线，互换定价便顺理成章。
...
交换利息的动机，可由一个典型场景说明。设想一家制造业企业借入一笔**浮动利率**贷款（利息随市场利率上下浮动）。企业担心未来加息会推高利息支出，而其所需的是可预测、可纳入预算的固定成本。与此同时，市场另一方可能存在需求相反的机构：它持有固定利率资产，却预期利率上行，希望将收入转为浮动。两者需求互补：制造业企业签订一笔"付固定、收浮动"的互换，**收到的浮动恰好抵消其贷款应付的浮动，剩余净支出便转化为一个固定利率**。企业并未提前偿还该笔浮动贷款（提前偿还需动用大笔现金，且可能触发违约金），仅凭一纸合约即将浮动利率敞口转换为固定。由此可见，互换的实质在于：**它将利率敞口视为可单独打包、单独转让的对象，从底层资产负债中剥离出来加以交易。** 改变利率敞口无需变动本金，只需就敞口本身进行处理——这正是 15.2.3 各类应用的共同内核。
...
在标准假设下（浮动腿按曲线重置、无信用利差），**浮动腿在重置日的现值有一个简洁的结论**：
